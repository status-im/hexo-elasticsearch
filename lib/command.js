...
module.exports = function(args, callback) {
  ...
  Promise.resolve(conf.esUser)
    ...
    .then((cleanPosts) => { /* Log number of posts found */
      info('%d pages and posts to index.', cleanPosts.length);
      return cleanPosts;
    })
    .then((cleanPosts) => { /* generate operation objects */
      var actionsAndPosts = cleanPosts.map((post, i) => [
        {index:{_index: conf.index, _type: 'post', _id: post._id}},
        post,
      ]);
