...
module.exports = async function(args, callback) {
  ...
  if (opts.delete) {
    try {
      ...
      err('Failed to delete index: %', err.message);
      dbg(err);
    }
  }
  ...
  let allPosts = pubPosts.concat(pages.toArray());
  /* extract fields to index */
  let cleanedPosts = allPosts.map(function(data) {
  ...
  let actionsAndPosts = cleanedPosts.map((post) => [
  ...
  let postsBatch = [].concat.apply([], actionsAndPosts);
